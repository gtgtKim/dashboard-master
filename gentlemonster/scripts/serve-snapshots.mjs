...
const DASHBOARD_COOKIE_NAME = process.env.DASHBOARD_COOKIE_NAME || 'gentlemonster_dashboard_auth';
  ...
  try {
    ...
    if (request.method === 'GET' && appPath === '/api/ga4-metrics') {
      if (!GA4_METRICS_ENABLED) {
        sendJson(response, 501, { status: 'disabled', error: 'GA4 metrics lookup is disabled for now.' });
        return;
      }
